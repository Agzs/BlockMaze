...
template<typename FieldT>
class add_pack_gadget_test : public gadget<FieldT> { // 基类和比较类组合，基本的note_gadget和comparison_gadget (value_s)
public:
    ...
    void generate_r1cs_witness(uint64_t v_old, uint64_t v_s, uint64_t v) { // 为变量生成约束
        ...
        std::cout << "****************\n this->pb.val(value_old_packed) = " << this->pb.val(value_old_packed);
        std::cout << "\n this->pb.val(value_s_packed) = " << this->pb.val(value_s_packed);
        std::cout << "\n this->pb.val(value_packed) = " << this->pb.val(value_packed) << "\n ****************\n";
                                                                   ...
                                                                   )
{
    ...
    add_pack_gadget_test<FieldT> g(pb); // 构造新模型
    g.generate_r1cs_constraints(); // 生成约束
    g.generate_r1cs_witness(value_old, value_s, value); // 为新模型的参数生成证明
                        ...
                        )
{
    ...
    add_pack_gadget_test<FieldT> ncmp(pb);
