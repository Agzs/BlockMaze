...
def cpp_val(s, log_radix=32):
    if log_radix == 8:
        ...
        raise
    return 'int_list_to_bits({0x%s}, %d)' % (', '.join(hexfmt % x for x in s), log_radix)
...
def generate_sha256_gadget_tests():
    ...
    print "h_data_bv = %s;" % cpp_val(left)
    print "tuple_data_bv = %s;" % cpp_val(right)
    print "tuple_data_bv2 = %s;" % cpp_val(right)
