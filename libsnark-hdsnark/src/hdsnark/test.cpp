...
int main() 
{
        ...
        for( ; ; ){
                ...
                if(input[0]==0&&input[1]==0){
                        ...
                        std::vector<bool> tuple_data_bv(256);  //1056*8 or?
                        //std::vector<bool> hash_coeff_bv(256);
                        std::vector<bool> data_coeff_bv(coeff_num);
                        ...
                        //v_tuple_data
                        for(int i=0;i<32;i++){                    //need to change 32
                               v_tuple_data.push_back(input[i+34]);   
                        }
                        //tuple_data_bv
                        tuple_data_bv=int_list_to_bits(v_tuple_data,8);
                        
                        std::cout << endl<<"input[66] = " << input[66] << std::endl;  //need to change 66
                        //v_data_coeff
                        for(int i=0;i<input[66];i++){                                   //need to change 66
                                v_data_coeff.push_back(input[i+67]);                    //need to change 67   +1
                        }
                        //data_coeff_bv
                        data_coeff_bv = int_list_to_bits(v_data_coeff,8);                
                        //result        
                        for(int i=0;i<input[66];i++){                                    //need to change 66
                                result+=input[34+i]*input[67+i];                        // not change 34  change 67
                        }
                        ...
                        std::vector<bool> test_coeff_data_bv(coeff_num);
