                                                                   ...
                                                                   )
{
    ...
    redeem_gadget<FieldT> redeem(pb); // 构造新模型
    redeem.generate_r1cs_constraints(); // 生成约束

    redeem.generate_r1cs_witness(note_old, note, cmtA_old, cmtA, value_s, sk_data); // 为新模型的参数生成证明
                        ...
                        )
{
    ...
    redeemTimeUse = gen_end.tv_sec - gen_start.tv_sec + (gen_end.tv_usec - gen_start.tv_usec)/1000000.0;
    printf("\n\nGen Redeem Proof Use Time:%fs\n\n", redeemTimeUse);

    // verify proof
    if (!proof) {
        printf("generate redeem proof fail!!!\n");
        return false;
    } else {
        //PrintProof(*proof);
        ...
        redeemVerTimeUse = ver_end.tv_sec - ver_start.tv_sec + (ver_end.tv_usec - ver_start.tv_usec)/1000000.0;
        printf("\n\nVer Redeem Proof Use Time:%fs\n\n", redeemVerTimeUse);
...
int main () {
    ...
    test_redeem_gadget_with_instance<default_r1cs_ppzksnark_pp>(value, value_old, value_s, keypair);
