...
r1cs_ppzksnark_proof<ppzksnark_ppT> generate_update_proof(r1cs_ppzksnark_proving_key<ppzksnark_ppT> proving_key,
                                                          const NoteS &note_s,
                                                          const Note &note_old,
                                                          const Note &note,
                                                          uint256 cmtS,
                     ...
                     char *RT)
{
    ...
    uint256 sn = uint256S(sn_string);
    uint256 r = uint256S(r_string);
    uint256 sns = uint256S(sns_string);
    uint256 rs = uint256S(rs_string);
